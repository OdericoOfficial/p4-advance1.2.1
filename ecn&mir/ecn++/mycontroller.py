...
# 增加设置threshold阈值的表，通过控制面下发。
def writeECNRules(p4info_helper, ingress_sw, egress_sw, mfs, len):
    table_entry = p4info_helper.buildTableEntry(
        table_name="MyEgress.qt",
        action_name="MyEgress.set_threshold",
        match_fields={
            "meta.thresh": mfs
        },
        action_params={
    ...
    ingress_sw.WriteTableEntry(table_entry)
    print("Installed ecn qthrehold rule on %s" % egress_sw.name )


def handle_pkt(pkt):
    print("Received response-----")
    print("ECN value:", pkt.payload.tos)
    if pkt.payload.tos == 3:
...
def show_tips():
    ...
    print("*                                              *")
    print("************************************************")
    sniff(filter="udp and port 4321", iface = 's1-eth1',
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        len1 = eval(input("please input the length of queue: "))
        ...
        writeECNRules(p4info_helper, ingress_sw=s1, egress_sw=s1, mfs=0, len=len1)
        ...
        writeECNRules(p4info_helper, ingress_sw=s2, egress_sw=s2, mfs=0, len=len1)
        ...
        writeECNRules(p4info_helper, ingress_sw=s3, egress_sw=s3, mfs=0, len=len1)
